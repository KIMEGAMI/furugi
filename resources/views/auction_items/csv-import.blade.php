        ...
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                ...
                <p class="mt-1 text-sm text-cyan-200">
                    古着システム用CSVと、他サービスのCSV変換登録をまとめて管理します。
                </p>
            </div>
    ...
    <div class="min-h-screen bg-slate-100 py-10">
        <div class="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
            ...
            @if($errors->has('yahoo_csv_file'))
                <div class="mb-6 rounded-2xl border border-red-200 bg-red-50 px-6 py-5">
                    <p class="font-bold text-red-700">{{ $errors->first('yahoo_csv_file') }}</p>
                </div>
            @endif

            @if($errors->has('mercari_shops_csv_file'))
                <div class="mb-6 rounded-2xl border border-red-200 bg-red-50 px-6 py-5">
                    <p class="font-bold text-red-700">{{ $errors->first('mercari_shops_csv_file') }}</p>
                </div>
            @endif
            ...
            <div class="space-y-6">
                <section class="rounded-3xl border border-slate-200 bg-white p-6 shadow">
                    <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                        <div>
                            ...
                            <h3 class="mt-2 text-xl font-black text-slate-950">古着システムCSVを一括登録</h3>
                            <p class="mt-2 text-sm font-semibold leading-6 text-slate-600">
                                古着システム形式のCSVをそのまま取り込みます。1行目はヘッダ行です。
                            </p>
                        </div>
                ...
                <section class="rounded-3xl border border-cyan-200 bg-white p-6 shadow">
                    <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                        <div>
                            ...
                            <p class="mt-2 text-sm font-semibold leading-6 text-slate-600">
                                ヤフオクの売上CSVを古着システム形式に変換し、SOLD商品として登録します。
                            </p>
                        </div>
                    ...
                    <form action="{{ route('auction-items.import.yahoo-auctions') }}" method="POST" enctype="multipart/form-data" class="mt-6 grid grid-cols-1 gap-4 lg:grid-cols-12 lg:items-end">
                        ...
                        <div class="lg:col-span-8">
                            ...
                            <input id="yahoo_csv_file" type="file" name="yahoo_csv_file" accept=".csv,.txt" class="mt-2 block w-full rounded-xl border border-slate-200 bg-slate-50 p-3 text-sm text-slate-600">
                            <p class="mt-2 text-xs font-semibold text-slate-500">
                                状態が「売上金」で、商品IDが入っている行だけを取り込みます。
                            </p>
                        </div>
                        <div class="lg:col-span-4">
                ...
                <section class="rounded-3xl border border-cyan-200 bg-white p-6 shadow">
                    <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                        <div>
                            <p class="text-xs font-black tracking-widest text-cyan-700">MERCARI SHOPS</p>
                            <h3 class="mt-2 text-xl font-black text-slate-950">メルカリShops売上明細CSVを変換して一括登録</h3>
                            <p class="mt-2 text-sm font-semibold leading-6 text-slate-600">
                                メルカリShops公式の売上明細CSVを古着システム形式に変換し、SOLD商品として登録します。
                            </p>
                        </div>
                        <button type="button" onclick="document.getElementById('mercariShopsCsvImportHelp').showModal()" class="rounded-lg border border-cyan-200 bg-cyan-50 px-4 py-3 text-sm font-black text-cyan-700 hover:bg-cyan-100">
                            メルカリShops CSVの項目説明
                        </button>
                    </div>
                    ...
                    <form action="{{ route('auction-items.import.mercari-shops') }}" method="POST" enctype="multipart/form-data" class="mt-6 grid grid-cols-1 gap-4 lg:grid-cols-12 lg:items-end">
                        @csrf
                        <div class="lg:col-span-8">
                            <label for="mercari_shops_csv_file" class="block text-sm font-black text-slate-700">メルカリShops売上明細CSVファイル</label>
                            <input id="mercari_shops_csv_file" type="file" name="mercari_shops_csv_file" accept=".csv,.txt" class="mt-2 block w-full rounded-xl border border-slate-200 bg-slate-50 p-3 text-sm text-slate-600">
                            <p class="mt-2 text-xs font-semibold text-slate-500">
                                明細種別が「購入」で、明細番号と商品名が入っている行だけを取り込みます。
                            </p>
                        </div>
                        <div class="lg:col-span-4">
                            <button type="submit" class="w-full rounded-xl bg-cyan-700 px-5 py-3 text-sm font-bold text-white shadow transition hover:bg-cyan-800">
                ...
                <section class="rounded-3xl border border-amber-200 bg-white p-6 shadow">
                    <div>
                        <p class="text-xs font-black tracking-widest text-amber-700">RAKUMA</p>
                        <h3 class="mt-2 text-xl font-black text-slate-950">ラクマCSV変換はサンプルCSV確認後に追加</h3>
                        <p class="mt-2 text-sm font-semibold leading-6 text-slate-600">
                            ラクマの売上CSVは、公式の列定義を確認できていないため未実装です。ヘッダ行を含むCSVサンプルを確認してから、確実な列対応で追加します。
                        </p>
                    </div>
                </section>
            </div>
                ...
                <div class="max-h-[85vh] overflow-y-auto bg-white p-6 text-slate-800">
                    <div class="flex items-start justify-between gap-4">
                        <div>
                            ...
                            <p class="mt-2 text-sm font-bold leading-6 text-slate-600">
                                1行目はヘッダ行として扱います。状態が「売上金」で、商品IDが入っている行だけを取り込みます。チャージ行や商品IDが「-」の行はスキップします。
                            </p>
                        </div>
                        ...
                        <table class="min-w-full border-collapse text-left text-sm">
                            <thead class="bg-slate-100 text-xs font-black text-slate-700">
                                <tr>
                                    ...
                                    <th class="border border-slate-200 px-3 py-2">古着システム項目</th>
                            ...
                            <tbody>
                                ...
                                <tr><td class="border border-slate-200 px-3 py-2 font-bold">取扱内容</td><td class="border border-slate-200 px-3 py-2 font-mono">title</td><td class="border border-slate-200 px-3 py-2">商品タイトルとして登録します。</td></tr>
                                <tr><td class="border border-slate-200 px-3 py-2 font-bold">取扱日</td><td class="border border-slate-200 px-3 py-2 font-mono">sold_at</td><td class="border border-slate-200 px-3 py-2">SOLD日として登録します。</td></tr>
                                <tr><td class="border border-slate-200 px-3 py-2 font-bold">決済金額</td><td class="border border-slate-200 px-3 py-2 font-mono">sold_price</td><td class="border border-slate-200 px-3 py-2">購入者が支払った金額として、売値に登録します。</td></tr>
                                ...
                                <tr><td class="border border-slate-200 px-3 py-2 font-bold">状態</td><td class="border border-slate-200 px-3 py-2 font-mono">status</td><td class="border border-slate-200 px-3 py-2">「売上金」の行だけをSOLDとして登録します。</td></tr>
                            </tbody>
                        </table>
                    </div>
                </div>
            ...
            <dialog id="mercariShopsCsvImportHelp" class="w-11/12 max-w-4xl rounded-2xl p-0 shadow-2xl backdrop:bg-slate-950/70">
                <div class="max-h-[85vh] overflow-y-auto bg-white p-6 text-slate-800">
                    <div class="flex items-start justify-between gap-4">
                        <div>
                            <h3 class="text-xl font-black text-slate-950">メルカリShops売上明細CSVの変換内容</h3>
                            <p class="mt-2 text-sm font-bold leading-6 text-slate-600">
                                1行目はヘッダ行として扱います。明細種別が「購入」で、明細番号と商品名が入っている行だけを取り込みます。キャンセルや特別付与はスキップします。
                            </p>
                        </div>
                    ...
                    <div class="mt-5 overflow-x-auto">
                        <table class="min-w-full border-collapse text-left text-sm">
                            ...
                            <tbody>
                                <tr><td class="border border-slate-200 px-3 py-2 font-bold">明細番号</td><td class="border border-slate-200 px-3 py-2 font-mono">management_id</td><td class="border border-slate-200 px-3 py-2">管理IDとして登録します。同じ明細番号が既にある場合はスキップします。</td></tr>
                                ...
                                <tr><td class="border border-slate-200 px-3 py-2 font-bold">販売手数料率（%）</td><td class="border border-slate-200 px-3 py-2 font-mono">sales_fee_rate</td><td class="border border-slate-200 px-3 py-2">列がある場合はその値を使い、なければ売上と手数料から計算します。</td></tr>
                                <tr><td class="border border-slate-200 px-3 py-2 font-bold">メルカリ便送料（税込）</td><td class="border border-slate-200 px-3 py-2 font-mono">shipping_fee</td><td class="border border-slate-200 px-3 py-2">出品者側の送料として登録します。購入者負担の「送料（税込）」は送料経費としては使いません。</td></tr>
                                <tr><td class="border border-slate-200 px-3 py-2 font-bold">販売利益</td><td class="border border-slate-200 px-3 py-2 font-mono">profit</td><td class="border border-slate-200 px-3 py-2">公式CSVの販売利益を実利益として登録します。</td></tr>
                            </tbody>
                        </table>
                    </div>
                </div>
