        ...
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
                ...
                <p class="mt-1 text-sm text-slate-300">
                    親ジャンル・小ジャンル・出品先の組み合わせから売上と実利益を分析します。
                </p>
            </div>
    ...
    <div class="py-8 text-white">
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div class="rounded-2xl border border-cyan-300/20 bg-slate-950/45 p-6 shadow-sm backdrop-blur-md">
                <form method="GET" action="{{ route('category-sales.index') }}"
                      class="flex flex-col gap-4 sm:flex-row sm:items-end">
                    <div class="w-full sm:max-w-xs">
                        <label for="month" class="block text-sm font-semibold text-slate-200">
                            集計月
                        </label>
                        <input id="month"
                               type="month"
                               name="month"
                               value="{{ $month }}"
                               class="mt-2 h-11 w-full rounded-xl border-cyan-300/20 bg-slate-900/80 text-white shadow-sm focus:border-cyan-300 focus:ring-cyan-300">
                    </div>

                    <button type="submit"
                            class="h-11 rounded-xl bg-cyan-500 px-5 text-sm font-black text-slate-950 hover:bg-cyan-400">
                        この月を表示
                    </button>

                    <a href="{{ route('category-sales.index') }}"
                       class="inline-flex h-11 items-center justify-center rounded-xl border border-cyan-300/30 px-5 text-sm font-black text-white hover:bg-cyan-400/10">
                        全期間
                    </a>
                </form>

                <p class="mt-3 text-sm text-slate-300">
                    表示期間: {{ $selectedMonth ? $selectedMonth->format('Y年n月') : '全期間' }}
                </p>
            </div>

            <div class="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                <div class="rounded-2xl border border-cyan-300/20 bg-slate-950/45 p-5">
                    <p class="text-sm text-slate-300">売上合計</p>
                    <p class="mt-2 text-2xl font-black text-white">
                        ¥{{ number_format($summary['sales']) }}
                    </p>
                </div>

                <div class="rounded-2xl border border-lime-300/30 bg-slate-950/45 p-5">
                    <p class="text-sm text-slate-300">実利益合計</p>
                    <p class="mt-2 text-2xl font-black {{ $summary['profit'] < 0 ? 'text-red-400' : 'text-lime-300' }}">
                        ¥{{ number_format($summary['profit']) }}
                    </p>
                </div>

                <div class="rounded-2xl border border-cyan-300/20 bg-slate-950/45 p-5">
                    <p class="text-sm text-slate-300">SOLD件数</p>
                    <p class="mt-2 text-2xl font-black text-white">
                        {{ number_format($summary['count']) }}件
                    </p>
                </div>

                <div class="rounded-2xl border border-cyan-300/20 bg-slate-950/45 p-5">
                    <p class="text-sm text-slate-300">利益率</p>
                    <p class="mt-2 text-2xl font-black text-white">
                        {{ number_format($summary['profit_rate'], 1) }}%
                    </p>
                </div>
            </div>
            ...
            <div class="mt-8 grid gap-6 lg:grid-cols-2">
                @include('sales.partials.ranking-table', [
                    'title' => '大ジャンル 売上ランキング',
                    'description' => '表示期間内の売上が高い大ジャンル上位10件です。',
                    'labelHeading' => '大ジャンル',
                ...
                @include('sales.partials.ranking-table', [
                    'title' => '小ジャンル 売上ランキング',
                    'description' => '表示期間内の売上が高い小ジャンル上位10件です。',
                ...
                options: {
                    ...
                    plugins: {
                        ...
                        tooltip: {
                            callbacks: {
                                label: function(context) {
                                    return Number(context.raw).toLocaleString() + '件';
                                }
                            }
                        }
                    },
                    scales: {
                        ...
                        y: {
                            ...
                            ticks: {
                                ...
                                precision: 0,
                                callback: function(value) {
                                    return Number(value).toLocaleString() + '件';
                                }
                            }
                        }
                    }
