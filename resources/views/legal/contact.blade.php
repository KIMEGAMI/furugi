...
>
    <h2>お問い合わせフォーム</h2>
    <p>FURUGI MANAGERの使い方、不具合、Premiumプラン、アカウント、個人情報の取り扱いに関するお問い合わせを受け付けています。</p>
    ...
    <form method="POST" action="{{ route('legal.contact.store') }}" class="not-prose mt-8 space-y-6 rounded-lg border border-slate-200 bg-slate-50 p-5">
        ...
        <div>
            ...
            <input
                ...
                class="mt-2 block w-full rounded-md border-slate-300 text-sm text-black shadow-sm focus:border-blue-500 focus:ring-blue-500"
            >
            @error('name')
        ...
        <div class="rounded-md border border-amber-200 bg-amber-50 p-4 text-xs font-semibold leading-6 text-amber-900">
            暴言、脅迫、差別的な表現、性的嫌がらせ、スパムを含む内容は送信できません。パスワード、APIキー、決済カード番号などの機密情報は入力しないでください。
        </div>
