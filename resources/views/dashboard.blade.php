<x-app-layout>
    ...
    <div
        ...
        style="background-image: linear-gradient(rgba(2, 6, 23, 0.58), rgba(2, 6, 23, 0.82)), url('{{ asset('images/bg.png') }}?v={{ time() }}');"
    >
        <div class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">

            <div class="mb-8 rounded-3xl border border-white/10 bg-black/45 p-8 shadow-2xl backdrop-blur-xl">
            ...
            <div class="mb-8 grid grid-cols-2 gap-4 lg:grid-cols-5">
                <div class="rounded-3xl border border-white/10 bg-black/45 p-5 shadow-xl backdrop-blur-xl">
                ...
                <div class="rounded-3xl border border-cyan-300/20 bg-cyan-400/10 p-5 shadow-xl backdrop-blur-xl">
                ...
                <div class="rounded-3xl border border-violet-300/20 bg-violet-400/10 p-5 shadow-xl backdrop-blur-xl">
                ...
                <div class="rounded-3xl border border-emerald-300/20 bg-emerald-400/10 p-5 shadow-xl backdrop-blur-xl">
                ...
                <div class="rounded-3xl border border-orange-300/20 bg-orange-400/10 p-5 shadow-xl backdrop-blur-xl">
            ...
            <div class="mb-8 grid grid-cols-1 gap-6 xl:grid-cols-2">
                <section class="rounded-3xl border border-white/10 bg-black/45 p-6 shadow-2xl backdrop-blur-xl">
                    <div class="mb-6 flex items-center justify-between">
                        <h2 class="text-xl font-black">{{ $year }}年 月別売上</h2>
                        <span class="text-xs font-bold text-slate-400">sold_at / sold_price</span>
                    </div>
                ...
                <section class="rounded-3xl border border-white/10 bg-black/45 p-6 shadow-2xl backdrop-blur-xl">
                    <div class="mb-6 flex items-center justify-between">
                        <h2 class="text-xl font-black">{{ $year }}年 月別利益</h2>
                        <span class="text-xs font-bold text-slate-400">sold_at / profit</span>
                    </div>

                    <div class="h-80">
                        <canvas id="monthlyProfitLineChart"></canvas>
                    </div>
                </section>
            </div>
                ...
                <section class="rounded-3xl border border-white/10 bg-black/45 p-6 shadow-2xl backdrop-blur-xl">
                    ...
                    <div class="mt-5 space-y-4">
                        <div class="rounded-2xl bg-white/10 p-5">
                            <div class="text-sm font-bold text-slate-400">販売手数料</div>
                            <div class="mt-2 text-2xl font-black">¥{{ number_format($totalSalesFee ?? 0) }}</div>
                        </div>

                        <div class="rounded-2xl bg-white/10 p-5">
                            <div class="text-sm font-bold text-slate-400">送料</div>
                            <div class="mt-2 text-2xl font-black">¥{{ number_format($totalShippingFee ?? 0) }}</div>
                        </div>
                    </div>
                </section>

                <section class="rounded-3xl border border-white/10 bg-black/45 p-6 shadow-2xl backdrop-blur-xl lg:col-span-2">
            ...
            <section class="mt-8 rounded-3xl border border-white/10 bg-black/45 p-6 shadow-2xl backdrop-blur-xl">
                ...
                @if (($recentItems ?? collect())->count() > 0)
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                        @foreach ($recentItems as $item)
                            ...
                            <a href="{{ route('auction-items.show', $item) }}" class="group overflow-hidden rounded-3xl border border-white/10 bg-white/10 shadow-xl transition hover:-translate-y-1 hover:bg-white/15">
                                <div class="aspect-[4/3] bg-slate-800">
                                    ...
                                    @else
                                        <div class="flex h-full items-center justify-center text-sm font-bold text-slate-500">
                                            NO IMAGE
                                        </div>
                                    @endif
                                </div>
                ...
                @else
                    <div class="rounded-2xl bg-white/10 p-8 text-sm font-bold text-slate-400">
                        最近更新した商品はありません。
                    </div>
                @endif
            </section>
    ...
    <script>
        ...
        const monthlySalesLineCanvas = document.getElementById('monthlySalesLineChart');
        const monthlyProfitLineCanvas = document.getElementById('monthlyProfitLineChart');
        ...
        if (monthlySalesLineCanvas) {
            new Chart(monthlySalesLineCanvas, {
                type: 'line',
                data: {
        ...
        if (monthlyProfitLineCanvas) {
            new Chart(monthlyProfitLineCanvas, {
                type: 'line',
                data: {
                    labels: monthlyChartLabels,
                    datasets: [
